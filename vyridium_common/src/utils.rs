use crate::{
    config::{COIN_DECIMALS, FEE_PER_ACCOUNT_CREATION, FEE_PER_KB, FEE_PER_TRANSFER},
...
pub fn format_vyridium(value: u64) -> String {
    format_coin(value, COIN_DECIMALS)
}

// Convert a XELIS amount from string to a u64
pub fn from_vyridium(value: impl Into<String>) -> Option<u64> {
...
pub fn from_coin(value: impl Into<String>, coin_decimals: u8) -> Option<u64> {
    ...
    let decimals: String = right_part
        .chars()
...
mod tests {
    ...
    fn test_xelis_format() {
        assert_eq!(format_vyridium(FEE_PER_ACCOUNT_CREATION), "0.00100000");
        assert_eq!(format_vyridium(FEE_PER_KB), "0.00010000");
        assert_eq!(format_vyridium(FEE_PER_TRANSFER), "0.00005000");
        assert_eq!(format_vyridium(COIN_VALUE), "1.00000000");
    }
    ...
    #[test]
    fn test_from_xelis() {
        let value = from_vyridium("100.123");
