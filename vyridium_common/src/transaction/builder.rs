...
use super::{
    ...
    BurnPayload, Reference, Role, SourceCommitment, Transaction, TransactionType, TransferPayload,
    TxVersion, EXTRA_DATA_LIMIT_SIZE, EXTRA_DATA_LIMIT_SUM_SIZE, MAX_TRANSFER_COUNT,
};
use crate::{
    account::CiphertextCache,
    api::DataElement,
    config::VYRIDIUM_ASSET,
    crypto::{
        elgamal::{
        ...
        Address, Hash, ProtocolTranscript, HASH_SIZE, SIGNATURE_SIZE,
    },
...
impl TransactionTypeBuilder {
    ...
    pub fn used_assets(&self) -> HashSet<Hash> {
        ...
        consumed.insert(VYRIDIUM_ASSET);
...
impl TransactionBuilder {
    ...
    pub fn estimate_size(&self) -> usize {
        ...
        // Range Proof
        let lg_n = (BULLET_PROOF_SIZE * (transfers_count + assets_used))
            .next_power_of_two()
...
mod tests {
    ...
    use crate::{
        crypto::{
            elgamal::{RISTRETTO_COMPRESSED_SIZE, SCALAR_SIZE},
            proofs::BULLET_PROOF_SIZE,
        },
    ...
    fn estimate_range_proof_size() {
        ...
        size += SCALAR_SIZE * 2;
        // G_vec len
        let lg_n = (BULLET_PROOF_SIZE * (transfers + assets))
            .next_power_of_two()
            .trailing_zeros() as usize;
