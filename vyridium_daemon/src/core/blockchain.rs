...
use vyridium_common::{
    ...
    utils::{calculate_tx_fee, format_vyridium},
    tokio::spawn_task,
    varuint::VarUint
};
use crate::{
...
impl<S: Storage> Blockchain<S> {
    ...
    async fn create_genesis_block(&self) -> Result<(), BlockchainError> {
        ...
        debug!("Registering XELIS asset: {} at topoheight 0", VYRIDIUM_ASSET);
        storage.add_asset(&VYRIDIUM_ASSET, AssetData::new(0, COIN_DECIMALS)).await?;
    ...
    pub async fn get_block_template_for_storage(&self, storage: &S, address: PublicKey) -> Result<BlockHeader, BlockchainError> {
        ...
        while let Some(TxSelectorEntry { size, hash, tx }) = tx_selector.next() {
            ...
            if let Err(e) = tx.verify(&mut chain_state).await {
                ...
                trace!("Selected {} (nonce: {}, fees: {}) for mining", hash, tx.get_nonce(), format_vyridium(tx.get_fee()));
                // TODO no clone
