...
use crate::{
    config::{BLOCK_TIME_MILLIS, DEV_FEES, DEV_PUBLIC_KEY},
    core::{
...
use vyridium_common::{
    api::{daemon::*, RPCTransaction, SplitAddressParams, SplitAddressResult},
    async_handler,
    block::{Block, BlockHeader, MinerWork},
    config::{MAXIMUM_SUPPLY, MAX_TRANSACTION_SIZE, VERSION, VYRIDIUM_ASSET},
    context::Context,
    crypto::Hash,
...
pub async fn get_block_type_for_block<S: Storage>(
    ...
    Ok(
        if blockchain
            .is_block_orphaned_for_storage(storage, hash)
            .await
        {
            BlockType::Orphaned
        } else if blockchain
            .is_sync_block(storage, hash)
            .await
            .context("Error while checking if block is sync")?
        {
            BlockType::Sync
        } else if blockchain
            .is_side_block(storage, hash)
            .await
...
async fn get_block_data<S: Storage>(
    blockchain: &Blockchain<S>,
    storage: &S,
    hash: &Hash,
) -> Result<
...
pub async fn get_block_response<S: Storage>(
    blockchain: &Blockchain<S>,
    storage: &S,
    hash: &Hash,
    block: &Block,
    ...
    let (dev_reward, miner_reward) = get_block_rewards(header.get_height(), reward)
    ...
    let value: Value = if include_txs {
        ...
        for tx_hash in header.get_txs_hashes() {
            total_size_in_bytes += storage
                ...
                .await
                .context(format!("Error while retrieving transaction {hash} size"))?;
        }
...
pub async fn get_transaction_response<S: Storage>(
    storage: &S,
    tx: &Arc<Transaction>,
    hash: &Hash,
    in_mempool: bool,
...
pub async fn get_transaction_response_for_hash<S: Storage>(
    ...
    match storage.get_transaction(hash).await {
        ...
        Err(_) => {
            ...
            get_transaction_response(storage, &tx.get_tx(), hash, true, Some(tx.get_first_seen()))
                .await
        }
    }
}
...
async fn get_assets<S: Storage>(context: &Context, body: Value) -> Result<Value, InternalRpcError> {
    ...
    let max = params
        .maximum_topoheight
        .unwrap_or_else(|| blockchain.get_topo_height());
    let assets = storage
        .get_partial_assets(maximum, skip, min, max)
        .await
...
fn get_range(
    ...
    maximum: u64,
    current: u64,
) -> Result<(u64, u64), InternalRpcError> {
...
async fn get_account_history<S: Storage>(
    ...
    loop {
        if let Some((topo, versioned_balance)) = version.take() {
            ...
            if params.asset == VYRIDIUM_ASSET {
...
async fn get_accounts<S: Storage>(
    ...
    let accounts = storage
        .get_registered_keys(maximum, skip, minimum_topoheight, maximum_topoheight)
        .await
        .context("Error while retrieving accounts")?
        .into_iter()
        .map(|key| key.to_address(mainnet))
        .collect::<Vec<_>>();
...
async fn is_account_registered<S: Storage>(
    ...
    let registered = if params.in_stable_height {
        storage
            .is_account_registered_at_topoheight(key, blockchain.get_stable_topoheight())
            .await
