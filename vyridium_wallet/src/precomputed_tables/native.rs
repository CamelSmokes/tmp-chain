...
use anyhow::Result;
use log::info;
use vyridium_common::crypto::ecdlp;
...
pub fn read_or_generate_precomputed_tables<P: ecdlp::ProgressTableGenerationReportFunction>(
    path: Option<String>,
    progress_report: P,
    l1: usize,
    ...
    if let Ok(mut file) = File::open(format!("{path}precomputed_tables_{l1}.bin")) {
        ...
        ecdlp::table_generation::create_table_file_with_progress_report(
            l1,
            precomputed_tables.get_mut(),
            progress_report,
        )?;
        File::create(format!("{path}precomputed_tables_{l1}.bin"))?
            .write_all(precomputed_tables.get())?;
    }
