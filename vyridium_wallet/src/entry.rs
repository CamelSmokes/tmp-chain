...
use vyridium_common::{
    api::{
        wallet::{
            EntryType as RPCEntryType, TransactionEntry as RPCTransactionEntry,
            TransferIn as RPCTransferIn, TransferOut as RPCTransferOut,
        },
        DataElement,
    },
...
impl TransactionEntry {
    ...
    pub fn summary(&self, mainnet: bool, storage: &EncryptedStorage) -> Result<String> {
        let entry_str = match self.get_entry() {
            EntryData::Coinbase { reward } => format!("Coinbase {} XELIS", format_vyridium(*reward)),
            ...
            EntryData::Incoming { from, transfers } => {
                ...
                for transfer in transfers {
                    if *transfer.get_asset() == VYRIDIUM_ASSET {
                        str.push_str(&format!(
                            "Received {} XELIS from {}",
                            format_vyridium(transfer.get_amount()),
                        ...
                        str.push_str(&format!(
                            "Received {} {} from {}",
                            format_coin(transfer.get_amount(), decimals),
                            transfer.get_asset(),
                            from.as_address(mainnet)
                        ));
                    }
            ...
            EntryData::Outgoing {
                ...
                let mut str = format!("Fee: {}, Nonce: {} ", format_vyridium(*fee), nonce);
                for transfer in transfers {
                    if *transfer.get_asset() == VYRIDIUM_ASSET {
                        str.push_str(&format!(
                            "Sent {} XELIS to {}",
