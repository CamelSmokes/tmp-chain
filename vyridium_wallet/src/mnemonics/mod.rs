...
use vyridium_common::{crypto::PrivateKey, serializer::Serializer};
...
fn verify_checksum(words: &Vec<String>, prefix_len: usize) -> Result<bool> {
    ...
    let checksum_word = words
        ...
        .context("Invalid checksum index")?;
...
pub fn key_to_words(key: &PrivateKey, language_index: usize) -> Result<Vec<String>> {
    let language = LANGUAGES
        .get(language_index)
        .context("Invalid language index")?;
...
pub fn key_to_words_with_language(key: &PrivateKey, language: &Language) -> Result<Vec<String>> {
    ...
    words.push(
        words
            ...
            .context("error no checksum calculation")?
            .clone(),
    );
...
#[cfg(test)]
mod tests {
    use vyridium_common::crypto::KeyPair;
