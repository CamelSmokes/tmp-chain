...
use vyridium_common::{
    async_handler,
    config::{COIN_DECIMALS, VERSION, VYRIDIUM_ASSET},
    crypto::{ecdlp, Address, Hashable},
    network::Network,
    prompt::{
        argument::{Arg, ArgType, ArgumentManager},
        command::{Command, CommandError, CommandHandler, CommandManager},
        LogLevel, ModuleConfig, Prompt, PromptError,
    },
    serializer::Serializer,
    transaction::{
        builder::{FeeBuilder, TransactionTypeBuilder, TransferBuilder},
        BurnPayload, Transaction,
    },
    utils::{format_coin, format_vyridium},
};
use vyridium_wallet::{config::DIR_PATH, wallet::Wallet};

#[cfg(feature = "network_handler")]
use vyridium_wallet::config::DEFAULT_DAEMON_ADDRESS;
...
#[command(styles = vyridium_common::get_cli_styles())]
pub struct Config {
...
async fn main() -> Result<()> {
    ...
    if let Some(path) = config.wallet_path {
        ...
        let precomputed_tables = Wallet::read_or_generate_precomputed_tables(
            config.precomputed_tables_path,
        ...
        command_manager.add_command(Command::new(
            ...
            CommandHandler::Async(async_handler!(recover_wallet)),
        ))?;
...
async fn xswd_handle_request_application(
    ...
    let mut message = format!(
        "XSWD: Allow application {} ({}) to access your wallet\r\n(Y/N): ",
        app_state.get_name(),
        app_state.get_id()
    );
    if signed {
        message = prompt.colorize_str(
            Color::BrightYellow,
            "NOTE: Application authorizaion was already approved previously.\r\n",
        ) + &message;
    }
    let accepted = prompt
...
async fn apply_config(
    wallet: &Arc<Wallet>,
    #[cfg(feature = "api_server")] prompt: &ShareablePrompt,
) {
    let config: Config = Config::parse();
        ...
        if let Ok(wallet) = context.get::<Arc<Wallet>>() {
            ...
            let balance = format!(
                ...
                prompt.colorize_string(
                    ...
                    &format_vyridium(
                        storage
                            .get_plaintext_balance_for(&VYRIDIUM_ASSET)
                            .await
                            .unwrap_or(0)
                    )
                ),
...
async fn change_password(manager: &CommandManager, _: ArgumentManager) -> Result<(), CommandError> {
    ...
    let old_password = prompt
        .read_input(
            prompt.colorize_str(Color::BrightRed, "Current Password: "),
            true,
        )
        .await
        .context("Error while asking old password")?;

    let new_password = prompt
        .read_input(
            prompt.colorize_str(Color::BrightRed, "New Password: "),
            true,
        )
        .await
...
async fn transfer(manager: &CommandManager, _: ArgumentManager) -> Result<(), CommandError> {
    ...
    let str_address = prompt
        .read_input(prompt.colorize_str(Color::Green, "Address: "), false)
        .await
        .context("Error while reading address")?;
    let address = Address::from_string(&str_address).context("Invalid address")?;

    let asset = prompt
        .read_hash(prompt.colorize_str(Color::Green, "Asset (default XELIS): "))
        .await
        .ok();

    let asset = asset.unwrap_or(VYRIDIUM_ASSET);
    ...
    // read amount
    let float_amount: f64 = prompt
        .read(prompt.colorize_string(
            Color::Green,
            &format!("Amount (max: {}): ", format_coin(max_balance, decimals)),
        ))
        .await
...
async fn transfer_all(
    ...
    let estimated_fees = wallet
        .estimate_fees(tx_type.clone())
        .await
        .context("Error while estimating fees")?;

    if asset == VYRIDIUM_ASSET {
    ...
    manager.message(format!(
        ...
        asset,
        address.to_string(),
        format_vyridium(estimated_fees)
    ));
...
async fn start_rpc_server(
    ...
    wallet
        .enable_rpc_server(bind_address, auth_config)
        .await
